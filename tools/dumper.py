...
class Dumper():
    ...
    def enable_dump_quanted(self, model, path="./dump", match_pattern=".*"):
        if model.training:
    ...
    def enable_dump_model(self, model, path="./dump", match_pattern=".*", hook_forward=_hook_forward):
        if model.training:
            print("error model must be eval when dump,call model.eval() before dump")
            exit(-1)
        if not os.path.exists(path):
    ...
    def analyse_layer_output(self, model, match_pattern=".*"):
        leaf_all_modules = _dfs_submodules(model)
    ...
    def save_out_analyse_log(self, save_log_path="Analyse_layer_output.log"):
