...
SIGMOID_Q31 = 31
SIGMOID_VENUSA_RAW_Q = 58
RUNTIME_SIGMOID_PLATFORMS = {PlatForm.arcs, PlatForm.mars, PlatForm.venusA}
...
def sigmoid_runtime_qbits(platform):
    if platform in {PlatForm.arcs, PlatForm.mars}:
        return SIGMOID_Q31
    if platform == PlatForm.venusA:
        return SIGMOID_VENUSA_RAW_Q
    raise RuntimeError(f"QSigmoid does not support platform {platform}")


def sigmoid_requant_to_qbits(value, src_q_bits, q_bits):
...
class QSigmoidFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, input_quantizer, output_bits):
        ...
        if QUANT_CONFIGS.platform == PlatForm.venus:    # Q11->Q15
            ...
            output_q_bits = SIGMOID_Q31 if output_bits is None else int(output_bits) - 1
            output_q = sigmoid_requant_to_qbits(
                runtime_output,
                sigmoid_runtime_qbits(QUANT_CONFIGS.platform),
                output_q_bits,
            )
...
class QSigmoid(QModuleTensor):
    ...
    def quantize_output(self, module, input, output):
        if (
            ...
            and QUANT_CONFIGS.platform in RUNTIME_SIGMOID_PLATFORMS
        ):
    ...
    def forward(self, x, *args, **kwargs):
        if torch.onnx.is_in_onnx_export():
            ...
            if isinstance(self.input_quantizer, nn.ModuleList):
                return QSigmoidFunction.apply(x, self.input_quantizer[0], self.output_quantizer.data_bits)
            else:
                return QSigmoidFunction.apply(x, self.input_quantizer, self.output_quantizer.data_bits)
