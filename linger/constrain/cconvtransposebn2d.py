...
class ConvTransposeBN2d(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        if self.training:
            ...
            new_conv_rlt = F.conv_transpose2d(
                                input,
                                new_weight,
                                new_bias,
                                self.conv.stride,
...
@register_cmodule(ConvTransposeBN2d)
class CConvTransposeBN2d(CModuleMixin, ConvTransposeBN2d):
    ...
    def extra_repr(self):
        s = nn.ConvTranspose2d.extra_repr(self.conv)
        s += ', '
